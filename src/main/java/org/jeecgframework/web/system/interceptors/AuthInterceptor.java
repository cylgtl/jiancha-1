...
public class AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && client.getUser()!=null ) {
				...
				boolean isOtherRole = true;
				List<TSRole> listRole = client.getUser().getRoles();
				for(TSRole role:listRole){
					String roleCode = role.getRoleCode();
					if(roleCode.equals("vistor")){
						request.setAttribute("vistor", "vistor");
						isOtherRole = false;
					}
					...
					if(roleCode.equals("rlzyk")){
						request.setAttribute("rlzy", "rlzyk");
						isOtherRole = false;
					}
					
					if(roleCode.equals("zzk")){
						request.setAttribute("zzk", "zzk");
						isOtherRole = false;
					}
					
					if(roleCode.equals("wsk")){
						request.setAttribute("wsk", "wsk");
						isOtherRole = false;
					}
				}
				
				request.setAttribute("isOtherRole", isOtherRole?1:0);
				String sessionDepartsCode = SessionUtils.getCurrentDepartsCode();
				request.setAttribute("sessionDepartsCode", sessionDepartsCode);
				...
				if(!ConvertUtils.isEmpty(functionId)){
					...
					Set<TSOperation> newall = new HashSet<TSOperation>();
					if(allOperation.size()>0){
						...
						String hasOperSql="SELECT operation FROM t_s_role_function fun, t_s_role_user role WHERE  " +
							"fun.functionid='"+functionId+"' AND fun.operation!=''  AND fun.roleid=role.roleid ";
									//+ "AND role.userid='"+client.getUser().getId()+"' ";
					...
					for (String dataRuleId : dataruleCodes) {
						TSDataRule dataRule = systemService.findEntity(TSDataRule.class, dataRuleId);
	...
	private boolean hasMenuAuth(HttpServletRequest request){
		...
		if (functionList == null) {
			functionList = systemService.findAll(TSFunction.class);
		}
