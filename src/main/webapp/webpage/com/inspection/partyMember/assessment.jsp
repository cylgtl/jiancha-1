		...
		<c:if test="${fn:length(lists)  <= 0 }">
			<tr>
				...
				<t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess" id="0_files"
				 	name="assessments[0].files" view="false" auto="true" uploader="fileController.do?saveFiles&sessionKey=0" 
				 	extend=".jpg;*,jpeg;*.png;*.gif;*.bmp;*.ico;*.tif;*.xls;*.doc;*.rar;*.txt;*.zip"  formData="id">
				</t:upload>
					
				</td>
				
				<input name="assessments[0].fileId" id="0_uplodeFile" type="hidden"/>
			</tr>
		</c:if>
		<c:if test="${fn:length(lists)  > 0 }">
			<c:forEach items="${lists}" var="assessment" varStatus="stuts">
				...
				<td align="left">
					<input name="assessments[${stuts.index}].fileId" id="${stuts.index }_uplodeFile" type="hidden" value="${assessment.fileId}"/>
		
					<c:choose>
   						<c:when test="${empty assessment.fileId}">
   				
   							<div class="form"><t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess"
				 name="assessments[${stuts.index}].files" view="false" auto="true" uploader="fileController.do?saveFiles&sessionKey=${stuts.index }" 
