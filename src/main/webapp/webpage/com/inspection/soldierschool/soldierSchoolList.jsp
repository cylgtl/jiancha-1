...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="soldierSchoolList" autoLoadData="true"  title="优秀士兵保送入学" actionUrl="soldierSchoolController.do?datagrid&currentDepartId=${currentDepart.orgCode}" idField="id" fit="true">
  ...
  <t:datagrid name="soldierSchoolList" autoLoadData="true"  title="优秀士兵保送入学" actionUrl="soldierSchoolController.do?datagrid&currentDepartId=${currentDepart.orgCode}" idField="id" fit="true">
   ...
   <t:dgCol title="操作" field="opt" width="100" align="center"></t:dgCol>
	...
	<t:dgFunOpt funname="lookDetail(id)" title="查看" />
	<t:dgToolBar title="录入" icon="icon-add"  url="soldierSchoolController.do?addorupdate" height="400"  funname="add"></t:dgToolBar>
     <t:dgFunOpt title="删除" funname="deleteConfirm(id,departId)"/> 
   <t:dgToolBar title="编辑"  icon="icon-edit"  url="soldierSchoolController.do?addorupdate" height="400" funname="update"></t:dgToolBar>
     </t:datagrid>
  </c:if>
  <div  style="padding: 3px; height: 40px">
    <div name="searchColums" style="float: left; padding-left: 15px;">
         ...
         <a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="soldierSchoolListsearch();" style="text-align: center;width: 140px">查询</a>
    </div>
</div>
  </div>
 </div>
 ...
 var admin = "${admin}";
	...
	function deleteConfirm(id,departId){
		if(admin || sessionDepartsCode.indexOf(departId) > -1){
			delObj('soldierSchoolController.do?del&id='+id,'soldierSchoolList');
		}else{
			alert("您没有权限处理其他连/科的数据");
		}
