...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="backBoneList" autoLoadData="true"  title="骨干配备" actionUrl="backboneController.do?datagrid&currentDepartId=${currentDepart.orgCode}" idField="id" fit="true">
  ...
  <t:datagrid name="backBoneList" autoLoadData="true"  title="骨干配备" actionUrl="backboneController.do?datagrid&currentDepartId=${currentDepart.orgCode}" idField="id" fit="true">
   ...
   <t:dgCol title="操作" field="opt" width="100" align="center"></t:dgCol>
   	 <t:dgToolBar title="录入" icon="icon-add"  operationCode="ggpb_add" url="backboneController.do?addorupdate" height="400"  funname="add"></t:dgToolBar>
	 <t:dgFunOpt funname="operateDetail(id,departId)"   operationCode="ggpb_operate" title="处理" />
	 <t:dgFunOpt funname="lookDetail(id)" title="查看" />
        <t:dgFunOpt title="删除" funname="deleteConfirm(id,departId)"/> 
      <t:dgToolBar title="编辑"  icon="icon-edit" operationCode="ggpb_edit"  url="backboneController.do?addorupdate" height="400" funname="update"></t:dgToolBar>
     </t:datagrid>
  </c:if>
  ...
  <div    style="padding: 3px; height: 40px">
    <div name="searchColums" style="float: left; padding-left: 15px;">
         ...
         <a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="backBoneListsearch();" style="text-align: center;width: 140px">查询</a>
    </div>
</div>
 ...
 var admin = "${admin}";
	...
	function deleteConfirm(id,departId){
		if(admin || sessionDepartsCode.indexOf(departId) > -1){
			delObj('backboneController.do?del&id='+id,'backBoneList');
		}else{
			alert("您没有权限处理其他连/科的数据");
		}
