...
<body style="overflow-y: hidden" scroll="no">
	<t:formvalid formid="formobj" dialog="true" usePlugin="password"
		layout="table" action="personnelSelectionController.do?addorupdateOperate&id=${residuald}">
		...
		<t:tabs id="tt" iframe="false" tabPosition="top" fit="false">
			<t:tab href="personnelSelectionController.do?viewMyselfInfo&id=${residuald}" icon="icon-search" title="基本资料" id="1"></t:tab>
			<t:tab href="personnelSelectionController.do?viewResidualRecommend&id=${residuald}" icon="icon-search" title="民族评议/推荐" id="2"></t:tab>
			<t:tab href="personnelSelectionController.do?viewAuditing&id=${residuald}" icon="icon-search" title="各级研究意见和结果" id="3"></t:tab>
			<t:tab href="personnelSelectionController.do?viewPerformance&id=${residuald}" icon="icon-search" title="个人平时表现" id="4"></t:tab>
			<t:tab href="personnelSelectionController.do?viewAssessment&id=${residuald}" icon="icon-search" title="考核结果" id="5"></t:tab>
			<t:tab href="personnelSelectionController.do?viewReport&id=${residuald}" icon="icon-search" title="举报" id="6"></t:tab>
		</t:tabs>
		</div>
	</t:formvalid>
  ...
  <table style="display: none">
	<tbody id="add_results_one_table_template">
		<tr>
				...
				<td align="left"><input  name="results[0].conclusion"  maxlength="500" type="text" value=""
					style="width: 250px;"></td>
				<%-- <td align="left">
	...
	<tbody id="add_results_table_template">
		<tr>
				...
				<td align="left"><input  name="results[#index#].opinion"  maxlength="500" type="text" value=""
					style="width: 250px;"></td>
				<td align="left"><input  name="results[#index#].conclusion"  maxlength="500" type="text" value=""
					style="width: 250px;"></td>
				<%-- <td align="left">
					<t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess"
	...
	<tbody id="add_bx_one_table_template">
		<tr>
				<td align="center"><input style="width: 50px;" type="checkbox" name="ck" /></td>
				<td align="left"><select name ="performances[0].bxType"> 
					<c:forEach items="${typeList}" var="type">
                    	<option value="${type.typecode}">${type.typename}</option>
                	</c:forEach>
				</select></td>
	...
	<tbody id="add_assessments_one_table_template">
		<tr>
				...
				<td align="left">
				<t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess"
				 name="assessments[0].file" view="false" auto="true" uploader="militrainingController.do?saveFiles" 
				 extend=".jpg;*.jpeg;*.png;*.gif;*.bmp;*.ico;*.tif;*.xls;*.doc;*.rar;*.txt;*.zip" id="file_Id" formData="id"></t:upload>
					</td>
				<td align="left"> 
					<select name ="assessments[0].conclusion">
						<c:forEach items="${conclusionList}" var="type">
		                    <option value="${type.typecode}">${type.typename}</option>
		                </c:forEach>
					</select>
				</td>
	...
	<tbody id="add_assessments_table_template">
		<tr>
				...
				<td align="left">
					<t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess"
				 name="assessments[#index#].file" view="false" auto="true" uploader="militrainingController.do?saveFiles" 
				 extend=".jpg;*.jpeg;*.png;*.gif;*.bmp;*.ico;*.tif;*.xls;*.doc;*.rar;*.txt;*.zip" id="file_Id" formData="id"></t:upload>
					</td>
				<td align="left"> 
					<select name ="assessments[#index#].conclusion">
						<c:forEach items="${conclusionList}" var="type">
		                    <option value="${type.typecode}">${type.typename}</option>
		                </c:forEach>
					</select>
