	...
	<tbody id="add_results_table">
		<c:if test="${fn:length(list)  <= 0 }">
			<tr id="row_0">
				...
				<td align="left"><input  name="results[0].option"  maxlength="500" type="text" value=""
					style="width: 250px;height:25px"></td>
				 <td align="left">
					 <t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess"
				 name="0_files" id="0_files" view="false" auto="true"  uploader="fileController.do?saveFiles&sessionKey=0" 
		  ...
		  <c:if test="${fn:length(list)  > 0 }">
			<c:forEach items="${list}" var="delivery" varStatus="stuts">
				...
				<td align="left">
					<t:upload dialog="false"   buttonText="上传附件" onUploadSuccess="uploadSuccess"
				 name="results[${stuts.index }].files" id="[${stuts.index }]_uplodfile" view="false" auto="true" uploader="fileController.do?saveFiles&sessionKey=${stuts.index }" 
				 extend=".jpg;*,jpeg;*.png;*.gif;*.bmp;*.ico;*.tif;*.xls;*.doc;*.rar;*.txt;*.zip"  formData="id"></t:upload>
					</td>
