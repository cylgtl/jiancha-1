...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="evaluationResidualList" autoLoadData="true"  title="官兵评残" actionUrl="evaluationResidualController.do?datagrid&currentDepartId=${currentDepart.orgCode}" idField="id" fit="true">
 ...
 <c:if test="${not empty manager || not empty admin}">
  <t:datagrid name="evaluationResidualList" autoLoadData="true"  title="官兵评残" actionUrl="evaluationResidualController.do?datagrid&currentDepartId=${currentDepart.orgCode}" idField="id" fit="true">
   ...
   <t:dgCol title="姓名" field="name" width="150" align="center"></t:dgCol>
   <t:dgCol title="基本信息" field="jobTitle" width="350" align="center"></t:dgCol>
   <t:dgToolBar title="录入" icon="icon-add"  url="evaluationResidualController.do?addorupdate" height="400"  funname="add"></t:dgToolBar>
   <t:dgCol title="操作" field="opt" width="100" align="center"></t:dgCol>
	 <t:dgFunOpt funname="operateDetail(id,departId)"    title="处理" />
	<t:dgFunOpt funname="lookDetail(id)" title="查看" />
   <t:dgFunOpt title="删除" funname="deleteConfirm(id,departId)"/> 
   <t:dgToolBar title="编辑"  icon="icon-edit"  url="evaluationResidualController.do?addorupdate" height="400" funname="update"></t:dgToolBar>
     </t:datagrid>
  </c:if>
  ...
  <div style="padding: 3px; height: 40px">
    <div name="searchColums" style="float: left; padding-left: 15px;">
         ...
         <a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="evaluationResidualListsearch();" style="text-align: center;width: 140px">查询</a>
    </div>
</div>
  </div>
 </div>
 ...
 var admin = "${admin}";
	...
	function deleteConfirm(id,departId){
		if(admin || sessionDepartsCode.indexOf(departId) > -1){
			delObj('evaluationResidualController.do?del&id='+id,'evaluationResidualList');
		}else{
			alert("您没有权限处理其他连/科的数据");
		}
