...
<%@include file="/context/mytags.jsp"%>
  ...
  function addOnclickPerformances(){   
	  var count=$("#add_performances_table tr").length;
		...
		$.each( $("#add_performances_table").find("input:checked"), function( index, val ) {
			...
			$.ajax({
				//请求方式
				    type:'GET',
				    //发送请求的地址以及传输的数据
				    url:"adjustController.do?deletePerformances&id="+id,
				    //服务器返回的数据类型
				    dataType:'json',
...
<table border="0" cellpadding="2" cellspacing="0" id="performances_table" style="width: 900px">
	<tr bgcolor="#E6E6E6">
		<td align="center" style="width:50px;" bgcolor="#EEEEEE">序号</td>
		<td align="center" style="width:100px;" bgcolor="#EEEEEE">类型</td>
		<td align="center"  style="width:500px;" bgcolor="#EEEEEE">附件</td>
	</tr>
		<tbody id="add_performances_table">
		<c:if test="${fn:length(lists)  <= 0 }">
			<tr>
				<td align="left"><input style="width: 50px;margin-left: 60px;" type="checkbox" /></td>
				<td align="left" >
		...
		<c:if test="${fn:length(lists)  > 0 }">
			<c:forEach items="${lists}" var="performance" varStatus="stuts">
				...
				<input style="width: 50px;margin-left: 60px;" type="checkbox" value="${performance.id}"/></td>
				<td align="left">
