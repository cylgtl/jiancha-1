    	...
    	$.each( $("#add_recommends_table").find("input:checked"), function( index, val ) {
			...
			$.ajax({
				//请求方式
				    type:'GET',
				    //发送请求的地址以及传输的数据
				    url:"adjustController.do?deleteRecommend&id="+id,
				    //服务器返回的数据类型
				    dataType:'json',
	...
	<tbody id="add_recommends_table">
		...
		<c:if test="${fn:length(lists)  > 0 }">
			<c:forEach items="${lists}" var="recommend" varStatus="stuts">
				...
				<td align="left"  style="width:100px;text-align:center;" >
					<input class="Wdate" onClick="WdatePicker()" value="<fmt:formatDate value='${recommend.beginTime}' type="date" pattern="yyyy-MM-dd"/>"  style="width: 100px" id="beginTime" name="recommends[${stuts.index }].beginTime" ignore="ignore"/>
				</td>
				<td align="left"  style="width:100px;text-align:center;" >
				<input class="Wdate" onClick="WdatePicker()"  value="<fmt:formatDate value='${recommend.endTime}' type="date" pattern="yyyy-MM-dd"/>" style="width: 100px" id="endTime" name="recommends[${stuts.index }].endTime" ignore="ignore"/>
				</td>
				<%-- <td align="left"  style="width:200px;text-align:center;" >
